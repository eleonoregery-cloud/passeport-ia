...
const usesShowIf = a => !['Non', 'Je ne sais pas'].includes(a.aiUse);
const chatbotSubShowIf = a => a.chatbot !== 'Non';
const deepfakeLabelShowIf = a => a.syntheticMedia !== 'Non';
const trainingSubShowIf = a => a.training !== 'Non';
...
const STEPS = [
  { title: 'Profil de votre entreprise', intro: 'Ces informations adaptent vos recommandations, sans modifier votre score.', screens: [[
    ...
    ['role', 'Comment votre entreprise utilise-t-elle principalement l’IA ?', ['Utilise des outils créés par d’autres entreprises', 'Crée ou vend ses propres outils IA', 'Ajoute des outils IA à ses produits ou services', 'Plusieurs de ces activités', 'Je ne sais pas'], true],
  ...
  { title: 'Maîtrise de l’IA - article 4', intro: 'Évaluez la formation des personnes qui utilisent ou supervisent l’IA.', screens: [[
    ...
    ['risks', 'Quels risques cette sensibilisation abordait-elle ?', ['Erreurs et hallucinations', 'Vérification humaine', 'Données personnelles et confidentialité', 'Secrets d’affaires et cybersécurité', 'Biais et discrimination', 'Propriété intellectuelle', 'Transparence des contenus', 'Usages interdits', 'Aucun de ces sujets', 'Je ne sais pas'], true, trainingSubShowIf],
    ['adaptedTraining', 'Le contenu était-il adapté à vos métiers, outils et responsabilités ?', ['Oui, parcours adaptés', 'Partiellement', 'Non, uniquement général', 'Aucune formation', 'Je ne sais pas'], false, trainingSubShowIf],
    ['evidence', 'Disposez-vous de preuves des mesures mises en place ?', ['Oui, programme, dates, participants et attestations', 'Oui, preuves incomplètes', 'Non', 'Je ne sais pas'], false, trainingSubShowIf]
  ]]},
  { title: 'Transparence - article 50', intro: 'Examinez vos interactions et contenus destinés au public.', screens: [[
    ...
    ['chatbotNotice', 'Les personnes sont-elles clairement informées dès la première interaction ?', ['Oui, claire et visible', 'Mention peu visible', 'Non', 'Je ne sais pas', 'Non concerné'], false, chatbotSubShowIf],
    ['assistantGuide', 'Une notice présente-t-elle son rôle, ses limites et un contact humain ?', ['Oui', 'Partiellement', 'Non', 'Je ne sais pas', 'Non concerné'], false, chatbotSubShowIf]
  ], [
    ['syntheticMedia', 'Publiez-vous des images, vidéos ou sons générés par IA pouvant sembler authentiques ?', ['Oui, régulièrement', 'Oui, occasionnellement', 'Non', 'Je ne sais pas']],
    ['deepfakeLabel', 'Ces contenus sont-ils clairement signalés comme créés ou modifiés par IA ?', ['Oui, systématiquement', 'Dans certains cas', 'Non', 'Je ne sais pas', 'Non concerné'], false, deepfakeLabelShowIf],
  ...
  { title: 'Gouvernance et contrôle', intro: 'Mesurez votre capacité à encadrer et à démontrer vos pratiques.', screens: [[
    ['policy', 'Disposez-vous d’une charte IA ? (un document interne qui fixe les règles et bonnes pratiques d’usage de l’IA dans votre entreprise)', ['Oui, diffusée et appliquée', 'Oui, mais incomplète', 'En préparation', 'Non', 'Je ne sais pas']],
...
const PACKS = {
  ...
  formation: { tag: 'Offre complémentaire', name: 'Formation Article 4', price: '510 € HT / participant', desc: 'Une journée pour former vos équipes et documenter leur sensibilisation à l’IA.', mailto: "mailto:contact@passeport-ia.fr?subject=Formation%20Article%204%20-%20demande%20d'information", programUrl: '/programme-formation-article-4.html' }
};
...
function App() {
  ...
  const current = STEPS[step]; const lastStep = STEPS.length - 1; const lastScreen = current.screens.length - 1; const questions = current.screens[screen].filter(([,,,,showIf]) => !showIf || showIf(answers));
  ...
  if (result) {
    ...
    const primaryKey = recommendPack(negative);
    const pack = PACKS[primaryKey];
    const needsFormation = primaryKey !== 'formation' && negative.some(i => i.key === 'training');
    const downloadReport = () => downloadReportPdf({ contact: { company: answers.company, firstName: answers.firstName, lastName: answers.lastName, email: answers.email, sector: answers.sector, date: Date.now() }, result, logoUrl });
